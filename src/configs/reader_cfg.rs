...
impl SettingsReader{
    pub fn new(file_name: &str, app_prefix: &str)->SettingsReader{
        ...
        let redis: RedisConfig = RedisConfig::new(hsettings.clone().try_into::<HashMap<String, String>>().unwrap());
        let log: LogConfig = LogConfig::new(hsettings.clone().try_into::<HashMap<String, String>>().unwrap());
        SettingsReader{
            redis,
            log
        }
...
fn read_env(app_prefix: &str)-> Config{
    let mut settings = config::Config::default();
    settings.merge(config::Environment::with_prefix(app_prefix)).unwrap();
    settings
}